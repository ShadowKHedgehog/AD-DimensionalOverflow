...
export const MachineHandler = {
  ...
  get baseIMCap() {
    return (Decimal.pow(Decimal.pow(Decimal.clampMin(this.uncappedRM.max(1).log10().sub(1000), 0), 2))
      .times((Decimal.pow(Decimal.clampMin(this.uncappedRM.max(1).log10().sub(100000), 1), 0.2))),TimeStudy(282).effectOrDefault(1));
  },
  ...
  get IMshatterBoostCap() {
    return new Decimal("1e1000").times(Decimal.pow(1e50, Decimal.floor(player.exposes.sub(100).div(15).max(0))));
  },
